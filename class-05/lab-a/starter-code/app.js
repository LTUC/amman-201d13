...
function sumArray(sumArr) {
	...
	let nums = sumArr.reduce((acc, num) => {
		return (acc += `${num},`);
	}, '');
	...
	return [
		...
		`${nums.slice(
			0,
			nums.length - 1,
		)} was passed in as an array of numbers, and ${sum} is their sum.`,
	];
}
...
function multiplyAnyArray(dynamicArray) {
	//eslint-disable-line
	let multi = dynamicArray.reduce((acc, num) => {
		return (acc *= num);
	});
	let nums = dynamicArray.reduce((acc, num) => {
		return (acc += `${num},`);
	}, '');
